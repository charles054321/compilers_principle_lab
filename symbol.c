...
void TypeRelease(TYPE *a){
	...
	if(a.kind == ARRAY){
		...
		FieldList *p;
		for(p = (&a.structure).next;p != &a.structure;){
			FieldList *field = p;
			p = p.next;
...
bool SymbolInsert(Symbol *a){
	...
	list *l = &p.list;
	l.prev = SymbolTable + hash;
	l.next = (SymbolTable + hash).next;
	if(SymbolTable + hash != NULL) (SymbolTable + hash).next = l;
	if((SymbolTable + hash).next != NULL) (SymbolTable + hash).next.prev = l;
	l.prev = (stack + top).prev;
	l.next = stack + top;
	if((stack + top).prev != NULL) (stack + top).prev.next = l;
	if(stack + top != NULL) (stack + top).prev = l;
	return true;
}
